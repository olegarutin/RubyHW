...
p 'arr = (-2..5).to_a'
p 'arr.each { |x| x.negative? ? x = arr.max : x }'
arr = (-2..5).to_a
p arr.map { |x| x.negative? ? x = arr.max : x }
p '--------------'
...
p 'max.count'
max = []
[10, 10, 10, 3, 4, 2, 1, 5, 3].each_cons(3).each_with_index { |x, _i| max << x[1] if x[0] < x[1] && x[1] > x[2] }
p max.count
p '--------------'
...
p 'min.count'
min = []
[10, 10, 10, 3, 4, 2, 1, 5, 3].each_cons(3).each_with_index { |x, _i| min << x[1] if x[0] > x[1] && x[1] < x[2] }
p min.count
p '--------------'
...
p 'maximum.sort.last'
maximum = []
[10, 10, 10, 3, 4, 2, 1, 5, 3].each_cons(3).each_with_index { |x, _i| maximum << x[1] if x[0] < x[1] && x[1] > x[2] }
p maximum.max
p '--------------'
...
p 'minimum.min'
minimum = []
[10, 10, 10, 3, 4, 2, 1, 5, 3].each_cons(3).each_with_index { |x, _i| minimum << x[1] if x[0] > x[1] && x[1] < x[2] }
p minimum.min
p '--------------'
...
count = []
arr.each_with_index { |e, i| count << (i != (arr.size - 1) && e < arr[i + 1] ? e : ' ') }
p count.join.split(' ').size
p '--------------'
...
input = (-4..4).to_a
output = []
input.each { |x| x.positive? ? output << input[0] && output << x : output << x }
p output
p '--------------'
...
input = (-4..4).to_a
output = []
input.each { |x| x.negative? ? output << input[0] && output << x : output << x }
p output
p '--------------'
...
input = (-4..4).to_a
output = []
input.each { |x| x.positive? ? output << x && output << input[0] : output << x }
p output
p '--------------'
...
input = (-4..4).to_a
output = []
input.each { |x| x.negative? ? output << x && output << input[0] : output << x }
p output
p '--------------'
...
p 'arr = [3, -5, 1, 6, 11, -3, -5, 11, 2]'
p '([] << arr.index(arr.min) << arr.index(arr.max)).min'
p arr = [3, -5, 1, 6, 11, -3, -5, 11, 2]
p ([] << arr.index(arr.min) << arr.index(arr.max)).min
p '--------------'
...
p 'array.each { |x| result << (x == array.min ? 1 : ' ') }'
p 'result.join.split(' ').map(&:chars).map(&:count).max'
result = []
array = [1, 1, 3, -1, -6, -6, 1, -6]
array.each { |x| result << (x == array.min ? 1 : ' ') }
p result.join.split(' ').map(&:chars).map(&:count).max
p '--------------'
p '64.Дан целочисленный массив. Найти максимальное количество подряд идущих максимальных элементов.'
p 'result = []'
p 'array = [1, 1, 3, -1, -6, -6, 1, -6]'
p 'array.each { |x| result << (x == array.max ? 1 : ' ') }'
p 'result.join.split(' ').map(&:chars).map(&:count).max'
result = []
array = [1, 1, 3, 3, -1, -6, -6, 3, 3, 3, 1, -6]
array.each { |x| result << (x == array.max ? 1 : ' ') }
p result.join.split(' ').map(&:chars).map(&:count).max
p '--------------'
...
p 'arr = (-4..4).to_a'
multiply = []
arr.each { |x| multiply << (x.negative? ? x**2 : x**3) }
p multiply
arr = (-4..4).to_a
multiply = []
arr.each { |x| multiply << (x.negative? ? x**2 : x**3) }
...
arr = [2, 3, 1, 19, 2, 5, 1]
result = []
(1...arr.size).select { |x| result << x if arr[x] < arr[x - 1] }
p result, result.count
p '--------------'
...
arr = [2, 3, 1, 19, 2, 5, 1]
result = []
(1...arr.size).select { |x| result << x -= 1 if arr[x] > arr[x - 1] }
p result, result.count
p '--------------'
